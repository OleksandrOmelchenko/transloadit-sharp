			...
			try {
				...
				if (hasBody) {
					...
					foreach (var file in Data.Files) {
						...
						bytes.AddRange (Encoding.UTF8.GetBytes (String.Format (PreFormDataFileTemplate, boundary, file.Key, file.Key)).ToArray<byte> ());
						bytes.AddRange (file.Value);
					...
					Stream requestStream = request.EndGetRequestStream(request.BeginGetRequestStream(null,null)); 
